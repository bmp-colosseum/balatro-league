...
// the TO picks the outcome from a single list (each team's win scores, a forfeit
// win each way, or a void/double-DQ) and it saves on pick. The options are derived
// from the set's best-of so only reachable scores show. Preselects the recorded
// result via `current` (see setOutcomeValue).
...
export function SetOutcomeSelect({
  ...
  const a = teamAName.slice(0, 16);
  const b = teamBName.slice(0, 16);
